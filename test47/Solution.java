 ...
 * 并且把已有最大的丑数记做M，我们接下来分析如何生成下一个丑数。该丑数肯定是前面某一个丑数乘以2、3或者5的结果，
 * 我们首先考虑把已有的丑数乘以2、3和5，然后通过比较三个数的大小，找出三个树中最小的丑数存入集合。为了避免重复，
 * 如果最小的丑数等于乘以2的结果，i2++；如果最小的丑数等于乘以3的结果，i3++;如果最小的丑数等于乘以5的结果，
 * i5++（i2、i3和i5是集合的索引且初值为0，首先集合存入1）。这样得到的丑数在集合中是按照从小到大排序的。
 * @author 54060
 *
 */
